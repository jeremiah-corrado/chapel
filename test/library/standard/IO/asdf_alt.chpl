...
prototype module MLPageError {
  ...
  class CSR_part : CSR_base, readDeserializable, writeSerializable {
    ...
    override proc serialize(writer: fileWriter(?), ref serializer: ?st) throws {
      if (st == binarySerializer) {
        ...
        try! writer.write(offsets[0..#oSize]);
        ...
        ret += try! "oDom = %?, offsets = [%? ... %?], iDom = %?, indices = [%? ... %?], wDom = %?, weights = [%? ... %?]".format(oDom, offsets[0..#3], offsets[..oDom.highBound#-3], iDom, indices[0..#3], indices[..iDom.highBound#-3], wDom, weights[0..#3], weights[..wDom.highBound#-3]);
        ret += "}";
        writer.write(ret);
      }
    }
  ...
  inline proc compArrSize() : 2*int(64) { //returns (numVerts, numEdges)
    var elms : int(64) = (2*avgDensity+1)*4; //1 vertex + avgDensity neighbors and weights, everybody is 32-bit
  ...
  proc consumeFile() {
    ...
    coforall partID in 0..<numParts with (ref header, ref inFile, ref outFile, in myReader, in myWriter) {
      on Locales[partID % Locales.size] {
        ...
        var myoutFile = try! IO.open(filePrefix + ".out.csr", ioMode.rw);
        myWriter = try! myoutFile.writer(serializer = new binarySerializer(endian = ioendian.little, _structured = false), locking = false, hints = IO.ioHintSet.sequential);
